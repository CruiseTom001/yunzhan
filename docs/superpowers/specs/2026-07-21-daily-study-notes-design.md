...
> 日期：2026-07-21
> 范围：Web/桌面共用前端、账号服务 API、PostgreSQL 迁移
...
5. 用户点击“AI 润色”，浏览器或桌面端 WebView 直接请求 AI 供应商，返回润色文本。
6. 用户可以把润色结果应用到记录并保存。
...
- AI 请求由前端直连供应商，云栈服务器不接收 API Key。
- AI Base URL 仅允许 `https:`，且不能包含账号、查询参数或片段。
- 直连供应商依赖供应商允许浏览器跨域请求；不支持 CORS 的供应商会被浏览器拦截。
